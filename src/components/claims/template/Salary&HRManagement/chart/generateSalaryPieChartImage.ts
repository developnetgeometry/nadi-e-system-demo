...
import { SalaryPieChart } from "./SalaryPieChart.tsx";

/**
 * Generate a chart image (base64 PNG) from salary data for PDF embedding
 * @param salary SalaryData[]
 * @returns Promise<string> base64 image string
 */
export async function generateSalaryPieChartImage(salary) {
    // Prepare chart data: group by staff_position and count
   const positionGroups = {};
    salary.forEach(item => {
        ...
        positionGroups[position].total += item.staff_salary || 0;
    ...
    root.render(React.createElement(SalaryPieChart, { data: chartData, width: 650, height: 380 }));
