...
export function ClaimListMcmc() {
  ...
  const handleExport = () => {
    ...
    const exportData = filteredClaims.map((claim) => ({
      ...
      UpdatedAt: claim.updated_at || "N/A",
    }));
  ...
  const filteredClaims = useMemo(() => {
    ...
    return claims;
  }, [claimMCMCData, search, filterYear, filterQuarter, filterStatus, filterPaymentStatus, sortField, sortOrder]);
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Select value={filterYear || "all"} onValueChange={(v) => setFilterYear(v === "all" ? null : v)}>
          ...
          <SelectContent>
            ...
            {[...new Set(claimMCMCData?.map((c) => c.year))].map((year) => (
        ...
        <Select value={filterStatus || "all"} onValueChange={(v) => setFilterStatus(v === "all" ? null : v)}>
          ...
          <SelectContent>
            ...
            {[...new Set(claimMCMCData?.map((c) => c.claim_status.name))].map((s) => (
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead sortable sorted={sortField === "dusp" ? sortOrder : null} onSort={() => handleSort("dusp")}>DUSP Name</TableHead>
