...
export function getGroupById(groups: UserGroup[], id?: string): UserGroup | undefined {
...
export function isTpGroup(group?: UserGroup): boolean {
  return !!group && (group.group_name.toLowerCase().includes('tp') || group.group_name.toLowerCase().includes('tech partner'));
}
