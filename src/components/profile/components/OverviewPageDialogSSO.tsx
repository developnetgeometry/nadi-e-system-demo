...
import { updateSSOProfile } from "@/components/profile/hook/use-sso-profile";
import { useUserMetadata } from "@/hooks/use-user-metadata";

interface ProfileEditDialogSSOProps {
    open: boolean;
...
const ProfileEditDialogSSO: React.FC<ProfileEditDialogSSOProps> = ({
    open,
    onOpenChange,
    profileData,
    onSave,
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            // Call the updateSSOProfile function with the updated formState
            await updateSSOProfile({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[90vw] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>Edit Profile SSO</DialogTitle>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {(userType === "super_admin" || userGroup === 4) && (
                            <>
                                <div>
...
export default ProfileEditDialogSSO;