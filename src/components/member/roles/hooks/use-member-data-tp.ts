...
export const useFetchMembersTp = () => {
  return useQuery({
    queryKey: ["members"],
    queryFn: async () => {
      ...
      // Step 3: Fetch members from nd_member_profile where ref_id matches organizationId
      const { data: membersData, error: membersError } = await supabase
        ...
        .eq("ref_id.dusp_tp_id", organizationId) // Filter by organization ID
