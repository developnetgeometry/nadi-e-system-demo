...
export function useLeaveOffType() {
  ...
  const addMutation = useMutation({
    mutationFn: async (leaveOffType: Partial<LeaveOffType>) => {
      const { error } = await supabase.from("nd_leave_off_type").insert([leaveOffType]);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      const { error } = await supabase.from("nd_leave_off_type").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["leave-off-types"] });
  ...
  const updateLeaveOffType = async (id: number, data: Partial<LeaveOffType>) => {
